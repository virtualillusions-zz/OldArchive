    ...
    private void setupFrame() {
       ...
       JPanel imagePl = new JPanel() {
           ...
           //private Image bgImg =ImageCache.retrieveCachedImage("/com/tps1/data/images/logo.jpg");
            private Image bgImg =Toolkit.getDefaultToolkit().getImage(getClass().getResource("/data/images/logo.jpg"));
            @Override
...
///////////////////////////////////////////////////////////////////////////////
       ...
       Set displayHeight = new HashSet();
       Set displayWidth = new HashSet();
       int[][] display = null ;
       for (int i = 0; i < displays.length; i++) {
    	   displayHeight.add(displays[i].getHeight());     
    	   displayWidth.add(displays[i].getWidth());
    	   }  
      // display=new int[displayWidth.size()][displayHeight.size()];
       for (int h = 0; h < displayHeight.size(); h++) {
    	   for (int w = 0; w < displayWidth.size(); w++) {
        	   display = new int[w][h];
           }    	   
       }
       
       System.out.println("display = "+display[4][0]);
       System.out.println("height = "+displayHeight);
       System.out.println("width = "+displayWidth);

       ArrayList<DisplayMode> a = new ArrayList<DisplayMode>();
       a.addAll(displayHeight);
       //System.out.println(active);
        for (int i = 0; i < a.size(); i++) {
        	for (int j = 0; j < a.size(); j++) {
        		
        	}
        	   listCb.addItem(new DisplayWrapper(displays[i]).mode.getHeight());      
        }
